...
let viewFilter  = 'focus'; // 'focus' | 'all' | 'today' | 'week' | 'parking' — Focus opens first
...
// ── Focus Mode state ─────────────────────────────────────────
// Persisted in chrome.storage.local under `focusTimer`. The side panel keeps a
// local copy; background.js owns it while the panel is closed (via chrome.alarms).
const DEFAULT_TIMER = { active: false, ended: false, phase: 'work', paused: false, workMin: 25, breakMin: 5, pomodoros: 0, endTs: 0, remainingSec: 0, totalSec: 0, day: null };
let focusTimer = { ...DEFAULT_TIMER };
let focusCustom = { workMin: 25, breakMin: 5 }; // last custom duration entry
let focusDurationChoice = '25/5'; // '25/5' | '50/10' | 'custom'
let timerTick = null; // setInterval handle for the running display

// ── chrome.storage helpers ─────────────────────────────────
function getClearedDone() {
  return new Promise(r => chrome.storage.local.get('clearedDoneIds', d => r(d.clearedDoneIds ? new Set(d.clearedDoneIds) : new Set())));
}
function setClearedDone(set) {
  chrome.storage.local.set({ clearedDoneIds: Array.from(set) });
}

function getCompletedSubs() {
  return new Promise(r => chrome.storage.local.get('completedSubs', d => r(d.completedSubs || {})));
}
function setCompletedSubs(v) { chrome.storage.local.set({ completedSubs: v }); }

function getIdeaIds() {
  return new Promise(r => chrome.storage.local.get('ideaIds', d => r(new Set(d.ideaIds || []))));
}
function setIdeaIds(set) {
  chrome.storage.local.set({ ideaIds: Array.from(set) });
}

// Focus (`now`) task IDs persisted locally so focus tasks survive closing the
// panel even before the `now` label round-trips through Todoist.
function getNowIds() {
  return new Promise(r => chrome.storage.local.get('nowIds', d => r(new Set(d.nowIds || []))));
}
function setNowIds(set) {
  chrome.storage.local.set({ nowIds: Array.from(set) });
}

// ── Focus timer persistence ──────────────────────────────────
function getFocusTimer() {
  return new Promise(r => chrome.storage.local.get('focusTimer', d => r(d.focusTimer ? { ...DEFAULT_TIMER, ...d.focusTimer } : { ...DEFAULT_TIMER })));
}
function saveFocusTimer() {
  chrome.storage.local.set({ focusTimer });
}
function remainingSeconds() {
  if (!focusTimer.active) return 0;
  if (focusTimer.paused) return focusTimer.remainingSec;
  return Math.max(0, Math.round((focusTimer.endTs - Date.now()) / 1000));
}
function scheduleAlarm() {
  chrome.alarms.clear('focusTimer');
  if (focusTimer.active && !focusTimer.paused) {
    chrome.alarms.create('focusTimer', { when: focusTimer.endTs });
  }
}

// ── Focus timer actions ──────────────────────────────────────
function startFocusSession() {
  const workMin = focusDurationChoice === 'custom' ? focusCustom.workMin : (focusDurationChoice === '50/10' ? 50 : 25);
  const breakMin = focusDurationChoice === 'custom' ? focusCustom.breakMin : (focusDurationChoice === '50/10' ? 10 : 5);
  const totalSec = workMin * 60;
  focusTimer = { ...focusTimer, active: true, ended: false, phase: 'work', paused: false, workMin, breakMin, endTs: Date.now() + totalSec * 1000, remainingSec: totalSec, totalSec };
  saveFocusTimer();
  scheduleAlarm();
  renderList();
}
...
function startBreak() {
  const totalSec = focusTimer.breakMin * 60;
  focusTimer = { ...focusTimer, active: true, ended: false, phase: 'break', paused: false, endTs: Date.now() + totalSec * 1000, remainingSec: totalSec, totalSec };
  saveFocusTimer();
  scheduleAlarm();
  renderList();
}
...
function pauseResumeTimer() {
  if (focusTimer.paused) {
    const sec = focusTimer.remainingSec;
    focusTimer = { ...focusTimer, paused: false, endTs: Date.now() + sec * 1000 };
  } else {
    focusTimer = { ...focusTimer, paused: true, remainingSec: remainingSeconds() };
  }
  saveFocusTimer();
  scheduleAlarm();
  renderList();
...
function addFiveMinutes() {
  if (focusTimer.paused) {
    focusTimer.remainingSec += 300;
  } else {
    focusTimer.endTs += 300 * 1000;
  }
  focusTimer.totalSec += 300;
  saveFocusTimer();
  scheduleAlarm();
  renderList();
}
...
function stopTimer() {
  focusTimer = { ...focusTimer, active: false, ended: false, paused: false, phase: 'work' };
  saveFocusTimer();
  chrome.alarms.clear('focusTimer');
  renderList();
}

// Session-ended → keep `now` tasks for the next block.
function carryOver() {
  focusTimer = { ...focusTimer, active: false, ended: false, paused: false, phase: 'work' };
  saveFocusTimer();
  chrome.alarms.clear('focusTimer');
...
// Session-ended → clear Focus: untag unfinished `now` tasks and push them to This week.
function startFresh() {
  const friday = daysUntilNextMonday() - 2;
  const toClear = tasks.filter(t => t.now && !t.done);
  getNowIds().then(ids => { toClear.forEach(t => ids.delete(t.id)); setNowIds(ids); });
  toClear.forEach(t => {
    tasks = tasks.map(x => x.id === t.id ? { ...x, now: false, due: x.due == null ? friday : x.due } : x);
    apiFetch('PATCH', '/tasks', { id: t.id, now: false, ...(t.due == null ? { due: friday } : {}) }).catch(() => { synced = false; });
  });
  focusTimer = { ...focusTimer, active: false, ended: false, paused: false, phase: 'work' };
  saveFocusTimer();
  chrome.alarms.clear('focusTimer');
...
async function loadTasks() {
  ...
  renderList();
  try {
    const [raw, completedSubs, cleared, ideaIds, nowIds] = await Promise.all([
      apiFetch('GET', '/tasks'),
      getCompletedSubs(),
      getClearedDone(),
      getIdeaIds(),
    ...
    tasks = (raw || []).map(task => {
      ...
      const isNow = nowIds.has(task.id) || !!task.now;
      const extra = (completedSubs[task.id] || []).filter(cs => !task.subtasks.find(s => s.id === cs.id));
      let merged = extra.length ? { ...task, subtasks: [...task.subtasks, ...extra] } : task;
      if (isIdea) merged = { ...merged, isIdea: true };
      if (isNow) merged = { ...merged, now: true };
      return merged;
    });
...
async function apiToggleNow(task) {
  const newNow = !task.now;
  tasks = tasks.map(t => t.id === task.id ? { ...t, now: newNow } : t);
  getNowIds().then(ids => { if (newNow) ids.add(task.id); else ids.delete(task.id); setNowIds(ids); });
  renderList();
  try {
    await apiFetch('PATCH', '/tasks', { id: task.id, now: newNow });
    synced = true;
  } catch { synced = false; }
  renderFooter();
}

// Create a task already tagged `now` (from the Focus "add a task to focus on" field)
async function apiCreateFocus(title) {
  const account = filter !== 'all' ? filter : 'personal';
  const tempId = 'tmp-focus-' + Date.now();
  const temp = { id: tempId, title, account, done: false, parked: false, now: true, due: null, subtasks: [], details: null };
  taskCreationOrder.set(tempId, Date.now());
  tasks = [temp, ...tasks];
  getNowIds().then(ids => { ids.add(tempId); setNowIds(ids); });
  renderList();
  document.getElementById('s-focus-add-input')?.focus(); // keep adding without re-clicking
  try {
    const created = await apiFetch('POST', '/tasks', { title, account, done: false, now: true });
    tasks = tasks.map(t => t.id === tempId ? { ...temp, ...created, now: true } : t);
    taskCreationOrder.set(created.id, taskCreationOrder.get(tempId));
    taskCreationOrder.delete(tempId);
    getNowIds().then(ids => { ids.delete(tempId); ids.add(created.id); setNowIds(ids); });
    synced = true;
  } catch { synced = false; }
  // Preserve any text typed during the request across the re-render, keep focus
  const pending = document.getElementById('s-focus-add-input')?.value || '';
  renderList();
  const inp = document.getElementById('s-focus-add-input');
  if (inp) { inp.value = pending; inp.focus(); }
  renderFooter();
}
...
async function apiDelete(task) {
  ...
  getNowIds().then(ids => { if (ids.has(task.id)) { ids.delete(task.id); setNowIds(ids); } });
  try {
...
function renderHeader() {
  ...
  h.innerHTML = `
    ...
    <div class="s-view-tabs">
      <button class="s-view-tab focus-tab ${viewFilter === 'focus' ? 'is-on' : ''}" id="s-view-focus">Focus</button>
  ...
  ['focus', 'all', 'today', 'week', 'parking'].forEach(v => {
...
function makeTaskEl(task) {
  ...
  if (isExpanded) {
    ...
    editor.innerHTML = `
      <div class="s-editor-body">
        ...
        <textarea class="s-details-input" placeholder="Add notes" data-notes="${task.id}" spellcheck="false">${escHtml(task.details || '')}</textarea>
        <div class="s-pill-row">
          <button class="s-pill ${task.due === 0 ? 'is-on' : ''}" data-due="0" data-tid="${task.id}" title="Today">!</button>
          <button class="s-pill ${task.due === 1 ? 'is-on' : ''}" data-due="1" data-tid="${task.id}" title="Tomorrow">!!</button>
          <button class="s-pill ${task.due && task.due > 1 && task.due < daysUntilNextMonday() ? 'is-on' : ''}" data-due-week="${task.id}" title="This week">EOW</button>
          ...
          <button class="s-pill focus-pill ${task.now ? 'is-on' : ''}" data-focus-pill="${task.id}" title="${task.now ? 'Remove from Focus' : 'Add to Focus'}">＋ Focus</button>
          <button class="s-pill ${task.parked ? 'is-on' : ''} s-park-pill" data-park-pill="${task.id}" title="Park for later">Park</button>
  ...
  // Notes shown when collapsed — populated notes are always visible, never hidden
  if (task.details && !isExpanded && !task.done) {
    const notesEl = document.createElement('div');
    notesEl.className = 's-notes-collapsed';
    notesEl.textContent = task.details;
...
function startTick() {
  ...
  timerTick = setInterval(() => {
    ...
    const timeEl = document.querySelector('.s-timer-time');
    if (timeEl) timeEl.textContent = fmtTime(sec);
    const fill = document.querySelector('.s-progress-fill');
    if (fill && focusTimer.totalSec) fill.style.width = (100 * (1 - sec / focusTimer.totalSec)) + '%';
    if (sec <= 0) stopTick(); // background alarm + storage listener flip to ended state
  }, 1000);
}
function stopTick() { if (timerTick) { clearInterval(timerTick); timerTick = null; } }

function focusAddHTML() {
  return `
    <div class="s-focus-add">
      <span style="color:var(--text-3);flex-shrink:0">${ICO_PLUS}</span>
      <input id="s-focus-add-input" placeholder="Add a task to focus on…" autocomplete="off" />
    </div>`;
}

function focusTaskListHTML(nowTasks) {
  if (!nowTasks.length) {
    return `<div class="s-focus-empty">Nothing in focus yet. Add a task above, or tap ＋ Focus on any task.</div>`;
  }
  return `
...
function renderFocusView(listEl) {
  let nowTasks = tasks.filter(t => t.now);
  // Oldest first → newly added focus tasks land at the bottom of the list
  nowTasks.sort((a, b) => (taskCreationOrder.get(a.id) || 0) - (taskCreationOrder.get(b.id) || 0));
  const doneCount = nowTasks.filter(t => t.done).length;
  const total = nowTasks.length;
  const unfinished = total - doneCount;

  const wrap = document.createElement('div');
  wrap.className = 's-focus';
  ...
  if (focusTimer.ended) {
    ...
    wrap.innerHTML = `
      <div class="s-timer-card">
        <div class="s-timer-top">
          <div class="s-timer-phase">Focus block complete</div>
          <div class="s-timer-meta">${focusTimer.workMin} min · ${doneCount} of ${total} done · 🍅 ${focusTimer.pomodoros} today</div>
        </div>
        <button class="s-break-btn" data-focus-break>Start ${focusTimer.breakMin}-min break</button>
        <div class="s-new-session-label">New session</div>
        <div class="s-session-opts">
          <button class="s-session-opt s-tip" data-focus-carry data-tip="Keep your unfinished focus tasks for the next block.">Carry over</button>
          <button class="s-session-opt s-tip" data-focus-fresh data-tip="Clear focus. Unfinished tasks move to This week; completed stay done.">Start fresh</button>
        </div>
      </div>`;
  ...
  // ── Running (work or break) ──
  if (focusTimer.active) {
    const sec = remainingSeconds();
    const isBreak = focusTimer.phase === 'break';
    const pct = focusTimer.totalSec ? (100 * (1 - sec / focusTimer.totalSec)) : 0;
    wrap.innerHTML = `
      <div class="s-timer-card">
        <div class="s-timer-top">
          <div class="s-timer-phase ${isBreak ? 'is-break' : ''}">${isBreak ? 'Break' : 'Focus'}</div>
          <div class="s-timer-time">${fmtTime(sec)}</div>
          ${isBreak ? '' : `<div class="s-timer-meta">${doneCount} of ${total} done</div>`}
        </div>
        <div class="s-progress"><div class="s-progress-fill ${isBreak ? 'is-break' : ''}" style="width:${pct}%"></div></div>
        <div class="s-timer-controls">
          <button class="s-timer-btn" data-focus-pause>${focusTimer.paused ? 'Resume' : 'Pause'}</button>
          <button class="s-timer-btn" data-focus-add5>+5 min</button>
          <button class="s-timer-btn is-danger" data-focus-stop>Stop</button>
        </div>
        <div class="s-pomo">🍅 ${focusTimer.pomodoros} ${focusTimer.pomodoros === 1 ? 'pomodoro' : 'pomodoros'} today</div>
      </div>
      ${isBreak ? '' : focusAddHTML() + focusTaskListHTML(nowTasks)}`;
    listEl.appendChild(wrap);
    if (!focusTimer.paused) startTick(); else stopTick();
    return;
  }

  // ── Setup ──
  stopTick();
  const isCustom = focusDurationChoice === 'custom';
  const startMin = isCustom ? focusCustom.workMin : (focusDurationChoice === '50/10' ? 50 : 25);
  wrap.innerHTML = `
    <div class="s-focus-chips">
      <button class="s-focus-chip ${focusDurationChoice === '25/5' ? 'is-on' : ''}" data-focus-dur="25/5">
        <span class="s-focus-chip-time">25 / 5</span><span class="s-focus-chip-label">Pomodoro</span>
      </button>
      <button class="s-focus-chip ${focusDurationChoice === '50/10' ? 'is-on' : ''}" data-focus-dur="50/10">
        <span class="s-focus-chip-time">50 / 10</span><span class="s-focus-chip-label">Deep work</span>
      </button>
      <button class="s-focus-chip ${isCustom ? 'is-on' : ''}" data-focus-dur="custom">
        <span class="s-focus-chip-time">Custom</span><span class="s-focus-chip-label">Set your own</span>
      </button>
    </div>
    ${isCustom ? `
    <div class="s-focus-custom">
      <span>Focus</span><input type="number" min="1" max="180" data-focus-work value="${focusCustom.workMin}" /><span>min</span>
      <span>·</span>
      <span>Break</span><input type="number" min="1" max="60" data-focus-break-min value="${focusCustom.breakMin}" /><span>min</span>
    </div>` : ''}
    ${focusAddHTML()}
    ${focusTaskListHTML(nowTasks)}
    <button class="s-focus-start" data-focus-start ${unfinished === 0 ? 'disabled' : ''}>
      Start Focus Mode${unfinished ? ` · ${startMin} min` : ''}
    </button>`;
  listEl.appendChild(wrap);
}
...
function renderList() {
  ...
  // Focus Mode is its own view
  if (viewFilter === 'focus') {
    renderFocusView(listEl);
    if (!eventsAttached) setupEvents();
    return;
  }
  stopTick();
  ...
  addRow.innerHTML = `
    ...
    <div class="s-add-expanded" style="${addTaskHasValue ? '' : 'display:none'}">
      ...
      <div class="s-pill-row">
        <button class="s-pill ${lastAddTaskPills.due === 0 ? 'is-on' : ''}" data-add-due="0" title="Today">!</button>
        <button class="s-pill ${lastAddTaskPills.due === 1 ? 'is-on' : ''}" data-add-due="1" title="Tomorrow">!!</button>
        <button class="s-pill ${lastAddTaskPills.due && lastAddTaskPills.due > 1 && lastAddTaskPills.due < daysUntilNextMonday() ? 'is-on' : ''}" data-add-due-week title="This week">EOW</button>
        <div style="position:relative;display:inline-flex">
        ...
        <button class="s-pill s-park-pill" data-add-park title="Park for later" style="margin-left:auto">Park</button>
      </div>
    </div>
  `;
...
function setupEvents() {
  ...
  listEl.addEventListener('keydown', e => {
    // Focus add-a-task input: Enter
    if (e.target.id === 's-focus-add-input' && e.key === 'Enter') {
      e.preventDefault();
      const title = e.target.value.trim();
      if (title) { apiCreateFocus(title); e.target.value = ''; }
      return;
    }
  ...
  listEl.addEventListener('click', e => {
    ...
    // ── Focus Mode controls ──
    const durChip = e.target.closest('[data-focus-dur]');
    if (durChip) { focusDurationChoice = durChip.dataset.focusDur; renderList(); return; }
    if (e.target.closest('[data-focus-start]')) { startFocusSession(); return; }
    if (e.target.closest('[data-focus-pause]')) { pauseResumeTimer(); return; }
    if (e.target.closest('[data-focus-add5]')) { addFiveMinutes(); return; }
    if (e.target.closest('[data-focus-stop]')) { stopTimer(); return; }
    if (e.target.closest('[data-focus-break]')) { startBreak(); return; }
    if (e.target.closest('[data-focus-carry]')) { carryOver(); return; }
    if (e.target.closest('[data-focus-fresh]')) { startFresh(); return; }
    const focusRemove = e.target.closest('[data-focus-remove]');
    if (focusRemove) {
      const t = tasks.find(x => x.id === focusRemove.dataset.focusRemove);
      if (t) apiToggleNow(t);
      return;
    }
    ...
    // Focus pill in editor (tag/untag `now`)
    const focusPill = e.target.closest('[data-focus-pill]');
    if (focusPill) {
      const task = tasks.find(t => t.id === focusPill.dataset.focusPill);
      if (task) apiToggleNow(task);
      return;
    }
    ...
    // Click title → put in edit mode AND expand the editor
    const titleClick = e.target.closest('.s-task-title');
    if (titleClick) {
      const tid = titleClick.dataset.titleTid;
      if (!expandedTasks.has(tid)) {
        expandedTasks.add(tid);
        renderList();
        setTimeout(() => {
          const el = document.querySelector(`[data-title-tid="${tid}"]`);
          if (el) { el.focus(); moveCursorToEnd(el); }
        }, 0);
      }
      return;
  ...
  // Change: date pickers + custom focus durations
  listEl.addEventListener('change', e => {
    const workInp = e.target.closest('[data-focus-work]');
    if (workInp) {
      focusCustom.workMin = Math.max(1, Math.min(180, parseInt(workInp.value) || 25));
      return;
    }
    const breakInp = e.target.closest('[data-focus-break-min]');
    if (breakInp) {
      focusCustom.breakMin = Math.max(1, Math.min(60, parseInt(breakInp.value) || 5));
      return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load the persisted focus timer (it may have advanced while the panel was closed)
  getFocusTimer().then(t => {
    focusTimer = t;
    // Reset the pomodoro tally on a new day
    const today = new Date().toISOString().slice(0, 10);
    if (focusTimer.day !== today) {
      focusTimer = { ...focusTimer, day: today, pomodoros: 0 };
      saveFocusTimer();
    }
    // If a running work block elapsed while we were away, flip to ended
    if (focusTimer.active && !focusTimer.paused && remainingSeconds() <= 0) {
      if (focusTimer.phase === 'break') {
        focusTimer = { ...focusTimer, active: false, ended: false, phase: 'work' };
      } else {
        focusTimer = { ...focusTimer, active: false, ended: true, pomodoros: (focusTimer.pomodoros || 0) + 1 };
      }
      saveFocusTimer();
    }
    if (viewFilter === 'focus') renderList();
  });

  // Keep in sync with background.js (which owns the timer while the panel is closed)
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes.focusTimer) {
      focusTimer = { ...DEFAULT_TIMER, ...changes.focusTimer.newValue };
      if (viewFilter === 'focus') renderList();
    }
  });
