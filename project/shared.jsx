...
const accLookup = Object.fromEntries(ACCOUNTS.map((a) => [a.id, a]));

// Fixed "today" so the prototype is stable
const TODAY = new Date(2026, 4, 22); // May 22 2026, a Thursday
...
function fmtShort(d) { return `${DAYS[d.getDay()]}, ${MONTHS[d.getMonth()].slice(0,3)} ${d.getDate()}`; }
function fmtMD(d) { return `${MONTHS[d.getMonth()].slice(0,3)} ${d.getDate()}`; }
function addDays(d, n) { const r = new Date(d); r.setDate(r.getDate() + n); return r; }
function sameDay(a, b) { return a.getFullYear()===b.getFullYear() && a.getMonth()===b.getMonth() && a.getDate()===b.getDate(); }
function weekStart(d) { // Monday start
  const r = new Date(d);
...
function Checkbox({ checked, onChange, accent, size = "md", circle = false }) {
  const cls = ["cb"];
...
function AccountDot({ acc, big = false }) {
...
function EmojiRain({ duration = 2800, count = 56, onDone }) {
...
function TopBar({ route, setRoute, dateLabel, onHubTab }) {
...
Object.assign(window, {
  ...
  ACCOUNTS, accLookup, TODAY, DAYS, MONTHS,
  fmtLong, fmtShort, fmtMD, addDays, sameDay, weekStart,
  Icon, Checkbox, AccountDot, StatusBadge, EmojiRain, TopBar,
});
