/* global React, ACCOUNTS, TODAY, fmtShort, fmtMD, addDays, weekStart, sameDay, DAYS, MONTHS,
   Checkbox, AccountDot, StatusBadge, Icon, useState, useEffect, useMemo */

// ============================================================
// Habits + Health hub
// ============================================================

function HabitsHub({ state, setState, sub, setSub }) {
  const tabs = [
...
function HabitsTab({ state, setState }) {
  ...
  return (
    <div>
      ...
      {/* Transposed table: rows = habits, columns = days */}
...
function AddHabitModal({ onClose, state, setState }) {
  ...
  const save = () => {
    ...
    setState((s) => ({
      ...
      // Back-link: add this habit to each selected goal's habitIds
      goals: s.goals.map((g) =>
...
function AddGoalModal({ onClose, state, setState }) {
  ...
  const save = () => {
    ...
    setState((s) => ({
      ...
      // Link back: add this goal to each selected habit's goals array
...
function WamTab({ state, setState }) {
  const [selectedGoal, setSelectedGoal] = useState("all");
  // Default to last week — most useful view at the start of a new week.
  const [weekOffset, setWeekOffset] = useState(-1);
  const habitsById = Object.fromEntries(state.habits.map((h) => [h.id, h]));

  // -1 = last week, 0 = this week (in-progress)
  ...
  // mock per-habit weekly percents — slight delta between weeks for realism
  ...
  // Highlight whichever week is selected
  const highlightIdx = isLast ? 5 : 6;
