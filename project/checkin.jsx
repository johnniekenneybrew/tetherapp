/* global React, ACCOUNTS, accLookup, TODAY, fmtLong, fmtShort, fmtMD, addDays, DAYS, MONTHS,
   Checkbox, AccountDot, EmojiRain, Icon, useState, useEffect, useRef, useMemo */

// ============================================================
// Daily Check-In (desktop two-column)
// ============================================================

function DailyCheckIn({ state, setState, navigateTo }) {
  const today = TODAY;
  ...
  // Readiness per section (the prerequisites for marking complete)
  const prioritiesReady = priorities.length >= 1;
  const habitsReady = true; // user is always allowed to mark habits reviewed (even if they did none)
  ...
  // Trigger celebration once all four sections marked complete
  useEffect(() => {
  ...
  const addPriority = () => {
    ...
    setNewPriority("");
    // editing the list resets section completion
  ...
  return (
    <div className="page fade-in" style={{ maxWidth: 960 }}>
      ...
      {/* Bottom status (no big button — sections drive completion) */}
